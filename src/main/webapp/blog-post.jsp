    ...
    <section class="page-banner">
        <div class="container">
            <div class="page-banner-content text-center">
                <h1 data-aos="fade-up"><c:out value="${blogPost.title}"/></h1>
                <div class="breadcrumbs" data-aos="fade-up" data-aos-delay="100">
                    <ul>
                        <li><a href="${pageContext.request.contextPath}/">Trang Chủ</a></li>
                        <li><a href="${pageContext.request.contextPath}/blog">Blog</a></li>
                        <li><span><c:out value="${blogPost.title}"/></span></li>
                    </ul>
                </div>
            </div>
        </div>
                ...
                <div class="col-lg-8">
                    <article class="blog-post-full" data-aos="fade-up">
                        <div class="blog-post-full-img">
                            <img src="${pageContext.request.contextPath}${not empty blogPost.imageUrl ? blogPost.imageUrl : '/resources/images/default-blog-large.jpg'}"
                        ...
                        <div class="blog-post-full-content">
                            <div class="blog-post-meta">
                                <span><i class="fas fa-user"></i> <c:out value="${blogPost.author}"/></span>
                                <span><i class="fas fa-calendar-alt"></i> <fmt:formatDate value="${blogPost.createdDate}" pattern="dd/MM/yyyy HH:mm" /></span>
                                <c:if test="${not empty blogPost.category}">
                                    <span><i class="fas fa-folder"></i> <a href="${pageContext.request.contextPath}/blog?category=${fn:escapeXml(blogPost.category)}"><c:out value="${blogPost.category}"/></a></span>
                                </c:if>
                            </div>
                            <h2 class="blog-post-full-title"><c:out value="${blogPost.title}"/></h2>
                                ...
                                <div class="blog-post-tags">
                                    ...
                                    <div class="tagcloud">
                                        <c:forTokens items="${blogPost.tags}" delims="," var="tag">
                                            <a href="${pageContext.request.contextPath}/blog?tag=${fn:escapeXml(fn:trim(tag))}"><c:out value="${fn:trim(tag)}"/></a>
                                        </c:forTokens>
                                    </div>
                                </div>
                            </c:if>
                            ...
                            <c:if test="${not empty profile}">
                                <div class="blog-post-author-box">
                                    <div class="author-img">
                                        <img src="${pageContext.request.contextPath}${not empty profile.photoUrl ? profile.photoUrl : '/resources/images/default-profile-author.jpg'}"
                            ...
                            <!-- Comments Section - SỬA ĐỔI TẠI ĐÂY -->
                            <div class="comments-area" id="comments">
                                <h3 class="comments-title">Bình Luận (<c:out value="${fn:length(commentsList) ne null ? fn:length(commentsList) : 0}"/>)</h3>
                                <c:choose>
                                    <c:when test="${not empty commentsList}">
                                        <ul class="comment-list">
                                            <c:forEach items="${commentsList}" var="comment">
                                                <%-- Chỉ hiển thị comment gốc (parent_comment_id is null) --%>
                                                <c:if test="${comment.parentCommentId == null}">
                                                    <li class="comment-item" id="comment-${comment.id}">
                                                        <div class="comment-author-img">
                                                            <img src="https://ui-avatars.com/api/?name=${fn:replace(comment.authorName, ' ', '+')}&size=60&background=random" alt="<c:out value='${comment.authorName}'/>">
                                                        </div>
                                                        <div class="comment-content">
                                                            <div class="comment-meta">
                                                                <strong><c:out value="${comment.authorName}"/></strong>
                                                                <span><fmt:formatDate value="${comment.createdDate}" pattern="dd/MM/yyyy 'lúc' HH:mm"/></span>
                                                            </div>
                                                            <p><c:out value="${comment.content}"/></p>
                                                                <%-- <a href="#comment-form-area" class="reply-link" onclick="setReplyTo(${comment.id}, '${fn:escapeXml(comment.authorName)}')">Trả lời</a> --%>
                                                        </div>
                                                    </li>
                                                    <%-- Tìm và hiển thị các comment con --%>
                                                    <c:forEach items="${commentsList}" var="reply">
                                                        <c:if test="${reply.parentCommentId == comment.id}">
                                                            <li class="comment-item comment-reply" id="comment-${reply.id}">
                                                                <div class="comment-author-img">
                                                                    <img src="https://ui-avatars.com/api/?name=${fn:replace(reply.authorName, ' ', '+')}&size=50&background=random" alt="<c:out value='${reply.authorName}'/>">
                                                                </div>
                                                                <div class="comment-content">
                                                                    <div class="comment-meta">
                                                                        <strong><c:out value="${reply.authorName}"/></strong>
                                                                        <span><fmt:formatDate value="${reply.createdDate}" pattern="dd/MM/yyyy 'lúc' HH:mm"/></span>
                                                                        <em>trả lời <c:out value="${comment.authorName}"/></em>
                                                                    </div>
                            ...
                            <div class="comment-form-area" id="comment-form-area">
                                ...
                                <form action="${pageContext.request.contextPath}/blog/post" method="post" class="comment-form">
                                    ...
                                    <input type="hidden" name="parentCommentId" id="parentCommentIdField" value="">
                                    <div class="row">
                                        <div class="col-md-6 form-group mb-3">
                                            <label for="authorName" class="form-label">Họ tên *</label>
                                            <input type="text" id="authorName" name="authorName" class="form-control" placeholder="Tên của bạn" required>
                                        </div>
                                        <div class="col-md-6 form-group mb-3">
                                            <label for="authorEmail" class="form-label">Email (tùy chọn)</label>
                                            <input type="email" id="authorEmail" name="authorEmail" class="form-control" placeholder="Email của bạn">
                                        </div>
                                        <div class="col-12 form-group mb-3">
                                            <label for="commentContent" class="form-label">Nội dung bình luận *</label>
                                            <textarea id="commentContent" name="commentContent" class="form-control" rows="5" placeholder="Viết bình luận của bạn ở đây..." required></textarea>
                                        </div>
                                        <div class="col-12">
                                            <button type="submit" class="btn btn-primary">Gửi Bình Luận</button>
                                            <button type="button" id="cancelReplyButton" class="btn btn-secondary" style="display:none;" onclick="cancelReply()">Hủy Trả Lời</button>
                                        </div>
                                    </div>
                                </form>
                            </div>
                            <!-- KẾT THÚC SỬA ĐỔI Comments Section -->
                        </div>
                    </article>
                </div>
